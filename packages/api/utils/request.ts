...
import { StandingsService } from "@/api/services/standings";
...
export class ErgastClient {
  ...
  constructor(
    ...
    this.cache = new RedisCache(redisOptions);
  ...
  async fetchAllPaginated<T>(path: string, limit: number = 100): Promise<any> {
    ...
    while (true) {
      ...
      // initialize combined response with metadata from the first request
      if (!combinedResponse) {
